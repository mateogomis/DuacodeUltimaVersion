...
const Salas = () => {
  ...
  useEffect(() => {
    const fetchSalas= async () => {
      try {
        const response = await axios.get('http://localhost:8000/api/sedes/salas/');
  ...
  return (
    <section className="room-section">
      ...
      <div className="room-cards">
        {salas.map((sala) => (
          <div className="room-card" key={sala.id}>
            <div className="room-header">
              <h3>{sala.nombre}</h3>
              <a
                href={sala.imagen_url}
                target="_blank"
            ...
            <p><strong>Aforo:</strong> {sala.capacidad}</p>
            {/* <p><strong>Disponibilidad:</strong> {sala.availability}</p>
            <p><strong>Estado:</strong> {sala.status}</p> */}
          </div>
        ))}
      </div>
    </section>
